...
const form=reactive({organ:'lung',diagnosis:'',description:'',date:new Date().toISOString().slice(0,10)})
const error=ref(''), saved=ref(false), busy=ref(false)
watch(current,r=>{form.organ=r?.organId||'lung';form.diagnosis=r?.diagnosis||'';form.description=r?.description||'';form.date=r?.date||new Date().toISOString().slice(0,10);saved.value=false},{immediate:true})
function newRecord(){selected.value='';form.diagnosis='';form.description='';saved.value=false}
async function save(){
  if(!store.selectedPatientId)return
  busy.value=true;error.value='';saved.value=false
  try{
    const result=await store.saveReport({id:selected.value,patientId:store.selectedPatientId,organId:form.organ,
  ...
  <div class="record-layout">
    <section class="card">
      ...
      <form class="card-body record-form" @submit.prevent="save">
        ...
        <div class="record-row"><label class="label">Organ<select v-model="form.organ" class="select"><option v-for="(name,id) in organNames" :key="id" :value="id">{{ name }}</option></select></label><label class="label">Record date<input v-model="form.date" class="input" type="date" required /></label></div>
        ...
        <div class="record-actions"><button class="btn btn-primary" :disabled="busy"><Check :size="16"/> {{busy?'Saving…':'Save medical record'}}</button><button v-if="selected" type="button" class="btn btn-secondary" @click="remove"><Trash2 :size="16"/> Soft delete</button></div>
      </form>
    </section>

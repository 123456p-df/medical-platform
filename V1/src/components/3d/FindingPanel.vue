...
<template>
  <aside class="finding-panel">
    <div class="panel-kicker">{{ $t("AI Finding Marker") }}</div>
    <div class="panel-heading">
      <div>
        <h3>{{ $t(finding.label) }}</h3>
        <p>{{ $t(finding.side) }} {{ $t(finding.location.replaceAll('_', ' ')) }}</p>
      </div>
      <RiskBadge :level="finding.severity" />
    </div>
    <p class="panel-description">{{ $t(finding.description) }}</p>
    <div class="panel-actions">
      <button type="button" class="btn btn-primary" @click="emit('viewCt')">
        <Crosshair :size="16" /> {{ $t("View CT") }} </button>

...
<template>
  <div class="slicer-viewer">
    <div class="viewer-toolbar">
      <div class="toolbar-left">
        <span class="modality-badge">
          <ScanLine :size="14" /> {{ $t(examination.type) }}
        </span>
        <div class="study-copy">
          <strong>{{ $t(examination.organ) }} · {{ $t(examination.bodyPart) }}</strong>
          <span>{{ $t(isProjection ? 'Projection viewer' : 'Multiplanar reconstruction') }}</span>
        </div>
      </div>

      <div v-if="!isProjection" class="toolbar-center">
        <span class="toolbar-label">{{ $t("Slice") }}</span>
        <input
        ...
        />
        <span class="slice-count">{{ $t(currentSlice) }} / {{ $t(examination.sliceCount) }}</span>
      </div>

      <div class="toolbar-right">
        <label class="preset-select">
          <span class="sr-only">{{ $t("Window preset") }}</span>
          <select v-model="windowPreset" class="select">
            <option value="lung">{{ $t("Lung") }}</option>
            <option value="brain">{{ $t("Brain") }}</option>
            <option value="bone">{{ $t("Bone") }}</option>
            <option value="soft">{{ $t("Soft tissue") }}</option>
          </select>
        </label>
        <button type="button" class="viewer-button" :aria-label="$t('Zoom out')" @click="changeZoom(-0.2)">
          <Minus :size="15" />
        </button>
        <span class="zoom-value">{{ $t(Math.round(zoom * 100)) }}%</span>
        <button type="button" class="viewer-button" :aria-label="$t('Zoom in')" @click="changeZoom(0.2)">
          <Plus :size="15" />
        </button>
        <button type="button" class="viewer-button" :aria-label="$t('Reset zoom')" @click="zoom = 1">
          <Crosshair :size="15" />
        </button>
        <button type="button" :class="['viewer-button', { active: showGrid }]" :aria-label="$t('Toggle grid')" @click="showGrid = !showGrid">
          <Grid3X3 :size="15" />
        </button>
      </div>
    </div>
    ...
    <div :class="['viewer-grid', { projection: isProjection }]">
      <template v-if="isProjection">
        <div class="viewport projection-viewport">
          <div class="viewport-label">{{ $t("X-Ray Projection") }}</div>
          <SyntheticSlice
            modality="X-Ray"
      ...
      <template v-else>
        <div class="viewport axial-viewport">
          <div class="viewport-label">{{ $t("Axial") }}</div>
          <SyntheticSlice
        ...
        <div class="viewport coronal-viewport">
          <div class="viewport-label">{{ $t("Coronal") }}</div>
          <SyntheticSlice
            :modality="examination.type"
            orientation="coronal"
            :slice-index="currentSlice"
        ...
        <div class="viewport sagittal-viewport">
          <div class="viewport-label">{{ $t("Sagittal") }}</div>
          <SyntheticSlice
            :modality="examination.type"
        ...
        <div class="viewport volume-viewport">
          <div class="viewport-label">{{ $t("3D Volume") }}</div>
          <VolumePreview
            :examination="examination"
            :findings="activeFindings"
            :active-finding-id="activeFindingId"
    ...
    <div v-if="activeFinding && !isProjection" class="finding-strip">
      ...
      <strong>{{ $t(activeFinding.label) }}</strong>
      <span>{{ $t(activeFinding.side) }} {{ $t(activeFinding.location.replaceAll('_', ' ')) }}</span>
    </div>
  </div>
</template>

...
<template>
  <div class="page">
    <PageHeader
      ...
      :subtitle="$t(`Good morning, ${auth.session?.name ?? 'Doctor'}. Here is today's clinical queue.`)"
    >
      <template #actions>
        <SearchBar v-model="filters.search" :placeholder="$t('Search patient name or ID...')" />
      </template>
    </PageHeader>

    <section class="grid four-col stats-grid">
      <StatsCard :label="$t('Total Patients')" :value="totalPatients" note="Across all modules" :icon="Users" tone="teal" />
      <StatsCard :label="$t('Pending Review')" :value="pendingReview" note="Needs doctor action" :icon="Clock3" tone="amber" />
      <StatsCard :label="$t('Abnormal Findings')" :value="abnormalFindings" note="High priority cases" :icon="TriangleAlert" tone="red" />
      <StatsCard :label="$t('Today\'s Exams')" :value="todaysExams" note="Scheduled for review" :icon="CalendarDays" tone="blue" />
    </section>
        ...
        <div class="card">
          <div class="card-header">
            <div>
              ...
              <p class="muted">{{ $t("Review and open the latest imaging studies.") }}</p>
            </div>
          </div>
        ...
        <div v-if="selectedPatient" class="selected-content">
          <div class="selected-person">
            <span class="large-avatar" :style="{ background: selectedPatient.avatarColor }">
              {{ $t(selectedPatient.name.split(' ').map((part) => part[0]).join('')) }}
            </span>
            <div>
              <strong>{{ $t(selectedPatient.name) }}</strong>
              <span>{{ $t(selectedPatient.id) }}</span>
            </div>
          </div>
          <div class="selected-meta">
            <span>{{ $t(selectedPatient.age) }} {{ $t("years") }}</span>
          ...
          <div class="preview-caption">
            ...
            <span>{{ $t(selectedPatient.lastExamDate) }}</span>
          </div>

...
const tabs = [
  ...
  { label: 'AI Findings', name: 'doctor-patient-ai' },
...
function selectQueryExam() {
  const examId = String(route.query.exam ?? '')
  if (store.examinations.some((exam) => exam.id === examId)) store.activeExamId = examId
}

onMounted(loadPatient)
watch(patientId, loadPatient)
watch(() => route.query.exam, selectQueryExam)
</script>

<template>
  <div class="page patient-detail">
    <button type="button" class="back-link" @click="router.push({ name: 'doctor-patients' })">
      <ArrowLeft :size="16" /> {{ $t("Patient List") }} </button>
    ...
    <section v-if="selectedPatient" class="patient-hero card">
      <div class="patient-identity">
        ...
        <div>
          <div class="name-row">
            <h1>{{ $t(selectedPatient.name) }}</h1>
            <RiskBadge :level="selectedPatient.risk" />
          </div>
          <p>
            {{ $t(selectedPatient.id) }} · {{ $t(selectedPatient.age) }} {{ $t("years ·") }} {{ $t(selectedPatient.gender) }} {{ $t("· ABO") }} {{ $t(selectedPatient.bloodType) }} {{ $t("· Rh(D)") }} {{ $t(selectedPatient.rhType ?? 'Unknown') }}
          </p>
        </div>
      </div>
      <div class="patient-status">
        ...
        <span class="latest-label">{{ $t("Latest:") }} {{ $t(selectedPatient.modality) }} {{ $t(selectedPatient.organ) }}</span>
      </div>
    </section>
    ...
    <nav class="detail-tabs" :aria-label="$t('Patient record sections')">
      <RouterLink
        v-for="tab in tabs"
        :key="tab.name"
        :to="{ name: tab.name, params: { id: patientId }, query: store.activeExamId ? { exam: store.activeExamId } : {} }"
        class="detail-tab"
    ...
    <div v-if="store.loading" class="loading"> {{ $t("Loading patient record...") }} </div>

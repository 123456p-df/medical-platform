    ...
    <section class="human-card card">
      <div class="card-header">
        <div>
          ...
          <p class="muted">{{ $t("Select an organ to open its visualization.") }}</p>
        </div>
      </div>
      <div class="card-body">
        <DigitalHumanViewer
    ...
    <section class="organ-view">
      <template v-if="selectedOrganId === 'lung'">
        <div class="card">
          <div class="card-header">
            <div>
              <h3>{{ $t("Lung 3D Model") }}</h3>
              <p class="muted">{{ $t("Standard anatomical model with AI finding marker.") }}</p>
            </div>
            <span class="organ-chip"><Activity :size="14" /> {{ $t("Lung") }}</span>
          </div>
          <div class="card-body">
            <LungViewer
      ...
      <div v-else class="card future-organ">
        ...
        <p class="muted">{{ $t("The clinical module for this organ is available in a future release.") }}</p>
      </div>
    </section>
  </div>
</template>

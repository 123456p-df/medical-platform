...
<template>
  <div class="page">
    <PageHeader
      :title="$t('My Health')"
      :subtitle="$t(`Good morning, ${auth.session?.name ?? 'Patient'}. Here is your health overview.`)"
    />
      ...
      <article class="health-overview card">
        <div class="card-header">
          <div>
            ...
            <p class="muted">{{ $t("Your most recent doctor-reviewed examination.") }}</p>
          </div>
        </div>
        <div class="card-body">
          <div v-if="latestExam" class="latest-exam">
            ...
            <div class="latest-exam-copy">
              <span class="kicker">{{ $t(latestExam.type) }} · {{ $t(latestExam.bodyPart) }}</span>
              <h2>{{ $t(latestExam.organ) }}</h2>
              <p>{{ $t(formatDate(latestExam.date)) }}</p>
            </div>
            <StatusBadge :status="latestReviewedReport ? 'Reviewed' : 'Pending Review'" />
          </div>
          <div class="health-actions">
            <button type="button" class="btn btn-primary" @click="router.push({ name: 'patient-reports' })">
              <FileText :size="16" /> {{ $t("View Reports") }} </button>
            <button type="button" class="btn btn-secondary" @click="router.push({ name: 'patient-body' })">
              <Activity :size="16" /> {{ $t("My Body") }} </button>
          </div>
        </div>
      </article>
    ...
    <section class="patient-lower-grid">
      <div class="card">
        <div class="card-header">
          ...
          <button type="button" class="btn btn-sm btn-secondary" @click="router.push({ name: 'patient-examinations' })"> {{ $t("View all") }} <ChevronRight :size="14" />
          </button>
        </div>
      ...
      <div class="card">
        <div class="card-header">
          <div>
            ...
            <p class="muted">{{ $t("Your latest reviewed final report.") }}</p>
          </div>
          <CalendarDays :size="18" class="header-icon" />
        </div>
        <div class="card-body">
          <ReportCard v-if="latestReviewedReport" :report="latestReviewedReport" patient-facing />
          <div v-else class="empty-state">{{ $t("Your reviewed report will appear here.") }}</div>
        </div>
      </div>
    </section>
  </div>

"""
Distributed inference script for RadSight.
...
def parse_args():
    parser = argparse.ArgumentParser(description="RadSight distributed inference.")
    ...
    parser.add_argument("--do-sample", "--do_sample", type=bool, default=True,
...
def run_inference(model, tokenizer, batch, device, args):
    inputs = batch["inputs"]
    inputs = {
...
def main():
    ...
    for batch in tqdm(dataloader, desc=f"Rank {global_rank}/{world_size}",
        ...
        try:
            prediction = run_inference(model, tokenizer, batch, device, args)
        except Exception as e:
            traceback.print_exc()
        ...
        if global_rank == 0:
            tqdm.write(
                f"[id={sample_id}] modal={batch['modal']} | "
                f"GT: {batch['ground_truth']!r} | Pred: {prediction!r}"
            )

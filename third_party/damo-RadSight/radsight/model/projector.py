...
class SpatialPoolingProjector(nn.Module):
    def __init__(self, mm_hidden_size, hidden_size):
        ...
        self.num_patches_post = [num // 2 for num in self.num_patches_pre]
...
def build_vision_projector(config, mm_hidden_size, delay_load=False, **kwargs):
    # radsight projector only support image-wise operation now, i.e., prohibit the temporal aggregation
    ...
    if projector_type == "linear":
        ...
        return SpatialPoolingProjector(mm_hidden_size, hidden_size)
    if projector_type == 'identity':
        return IdentityMap()

    raise ValueError(f'Unknown projector type: {projector_type}')

def build_vision_projector_3d(config, mm_hidden_size, delay_load=False, **kwargs):
    # radsight projector only support image-wise operation now, i.e., prohibit the temporal aggregation
    projector_type_3d = getattr(config, 'mm_projector_type_3d', 'linear')
    hidden_size = config.text_config.hidden_size

    if projector_type_3d.startswith("spp"):
        return SpatialPoolingProjector(mm_hidden_size, hidden_size)
    if projector_type_3d == 'identity':
        return IdentityMap()

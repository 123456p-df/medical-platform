...
"""PyTorch RadSight vision encoder model."""
...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "configuration_radsight_encoder",
        osp.join(osp.dirname(__file__), "configuration_radsight_encoder.py"),
    )
    configuration_radsight_encoder = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(configuration_radsight_encoder)
    RadSightVisionEncoderConfig = getattr(
        configuration_radsight_encoder,
        "RadSightVisionEncoderConfig",
    )
...
class RadSightVisionEmbeddings(nn.Module):

    def __init__(self, config: RadSightVisionEncoderConfig):
        super().__init__()
        self.config = config
...
# Copied from transformers.models.clip.modeling_clip.CLIPMLP with CLIP->RadSight
class RadSightVisionMLP(nn.Module):
...
class RadSightVisionEncoderLayer(nn.Module):
    ...
    def __init__(self, config: RadSightVisionEncoderConfig):
        ...
        self.mlp = RadSightVisionMLP(config)
...
class RadSightVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, config: RadSightVisionEncoderConfig):
        ...
        self.layers = nn.ModuleList([RadSightVisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class RadSightVisionEncoderModel(PreTrainedModel):

    config_class = RadSightVisionEncoderConfig
    base_model_prefix = "radsight"
    main_input_name = "pixel_values"
    supports_gradient_checkpointing = True
    _no_split_modules = [
        "RadSightVisionEncoderLayer",
        "RadSightVisionEmbeddings",
    ]
    ...
    def __init__(self, config: RadSightVisionEncoderConfig):
        ...
        self.embeddings = RadSightVisionEmbeddings(config)
        self.encoder = RadSightVisionTransformerEncoder(config)
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, RadSightVisionMLP):

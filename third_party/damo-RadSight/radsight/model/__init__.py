...
from .radsight_qwen2 import RadSightQwen3ForCausalLM, RadSightQwen3Config


VLLMs = {
    "radsight_qwen3": RadSightQwen3ForCausalLM,
}

VLLMConfigs = {
    "radsight_qwen3": RadSightQwen3Config,
}
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", **kwargs):
    ...
    # Apple Silicon / non-CUDA runtimes cannot use flash_attention_2.
    config._attn_implementation = kwargs.pop('attn_implementation', "sdpa")
    if not hasattr(config, "mm_attn_implementation") or config.mm_attn_implementation == "flash_attention_2":
        config.mm_attn_implementation = config._attn_implementation
    ...
    # judge model type
    model_type = config.model_type if hasattr(config, "model_type") else kwargs.pop('model_type', "radsight_qwen2")
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        print('Loading RadSight from base model...')

        if 'qwen2' in model_base.lower():
            model = RadSightQwen3ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = RadSightQwen3ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        print('Loading additional RadSight weights...')
        ...
        if model_type in ['radsight', 'radsight_qwen2']:
            model = RadSightQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = RadSightQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        model.load_state_dict(mm_projector_weights, strict=False)
    elif 'radsight' in model_type:
        # NOTE: SFT model loading
        tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, token=token)

        if model_type in ['radsight_qwen2']:
            model = RadSightQwen3ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = RadSightQwen3ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
    else:

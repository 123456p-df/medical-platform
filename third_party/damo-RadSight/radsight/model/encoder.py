...
from .radsight_encoder import (RadSightVisionEncoderConfig,
    RadSightVisionEncoderModel, RadSightImageProcessor)
...
class CLIPVisionEncoder(nn.Module):
    ...
    def __init__(self, vision_encoder, args, delay_load=False):
        ...
        if not delay_load:
            self.attn_implementation = getattr(args, 'mm_attn_implementation', 'sdpa')
            self.load_model()
        else:
...
class RadSightVisionEncoder(nn.Module):
    ...
    def __init__(self, vision_encoder, args, delay_load=False):
        ...
        if not delay_load:
            ...
            # uncertain whether flash-attention-2 is supported during inference phase.
            self.attn_implementation = 'sdpa' # 'eager'
            self.cfg_only = RadSightVisionEncoderConfig.from_pretrained(self.vision_encoder_name)
    ...
    def load_model(self, args):
        ...
        self.image_processor = RadSightImageProcessor.from_pretrained(self.vision_encoder_name)

        # merge_size is fixed to 1 for STAGE1, STAGE1.5, STAGE2, STAGE3 in encoder and can be modified in connector.
        self.cfg_only = RadSightVisionEncoderConfig.from_pretrained(self.vision_encoder_name)

        self.vision_encoder = RadSightVisionEncoderModel.from_pretrained(
...
def build_vision_encoder(vision_encoder_cfg, **kwargs):
    ...
    if  'clip' in vision_encoder:
        ...
        vision_encoder = RadSightVisionEncoder(vision_encoder, args=vision_encoder_cfg, **kwargs)
    else:

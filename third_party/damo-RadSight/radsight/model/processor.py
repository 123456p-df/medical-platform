...
from radsight.constants import DEFAULT_IMAGE_TOKEN, IGNORE_INDEX
from radsight.mm_utils import load_video, load_images
from radsight.model.ct_processor import CTImageProcessor
...
class RadSightProcessorKwargs(ProcessingKwargs, total=False):
    _defaults = {
        "text_kwargs": {
            "padding": False,
        },
...
class RadSightProcessor(ProcessorMixin):
    r"""
    ...
    attributes = ["image_processor", "tokenizer", "image_processor_3d"]
    ...
    def __init__(self, image_processor=None, tokenizer=None, chat_template=None, **kwargs):
        ...
        self.image_proceesor_3d = CTImageProcessor()
    ...
    def process_images(
        ...
        **kwargs,
        ...
        if modal not in ["image", "video", "volume"]:
            raise ValueError(f"modal must be 'image' or 'video' or 'volume', got '{modal}'")
        # 3D Processor
        # image_inputs = self.image_processor(images=images, merge_size=merge_size, **kwargs)

        if modal in ["image", "video"]:
            # Use 2D image processor
            ...
            # Use 3D CT image processor
            image_inputs = self.image_proceesor_3d(
                images=images,
                # return_tensors="pt",
                **kwargs
            )
    ...
    def __call__(
        ...
        images: ImageInput = None,
        modal: str = "image",  # 新增modal参数
        merge_size: Optional[int] = 1,
        return_labels: bool = False,
        **kwargs: Unpack[RadSightProcessorKwargs],
    ) -> BatchFeature:
        """
        ...
        image_inputs = self.process_images(
            images=images,
            modal=modal,  # 传递modal参数
            merge_size=merge_size, 

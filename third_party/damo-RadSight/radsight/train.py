...
from radsight.constants import (IGNORE_INDEX,
    NUM_FRAMES, DEFAULT_IMAGE_TOKEN, STREAM_MAX_FRAMES,
    STREAM_START_TOKEN, STREAM_END_TOKEN)
from radsight.mm_utils import (load_images, load_video,
                                  tokenizer_multimodal_token, load_3D)
from radsight.model import *
from radsight.radsight_trainer import (
    RadSightTrainer, find_all_linear_names, get_peft_state_maybe_zero_3,
...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="radsight_qwen3", metadata={"help": "Model type selected in the list: " + ", ".join(VLLMs.keys())})
    model_path: Optional[str] = field(default="Qwen/Qwen3-VL-4B-Instruct")
    version: Optional[str] = field(default="v1", metadata={"help": "Version of the conversation template."})
    freeze_backbone: bool = field(default=False, metadata={"help": "Whether to freeze the LLM backbone."})
    # Connector Arguments
    mm_projector_type: Optional[str] = field(default='mlp2x_gelu')
    mm_projector_type_3d: Optional[str] = field(default='spp')
    pretrain_mm_projector: Optional[str] = field(default=None)
    pretrain_mm_projector_3d: Optional[str] = field(default=None)
    ...
    mm_vision_hidden_size: int = field(default=1152, metadata={"help": "Hidden size of vision encoder."})
    mm_vision_embed_dim: int = field(default=1152,metadata={"help": "Embed dim of vision encoder."})
    ...
    # Vision tower 3D Arguments
    ct_vision_encoder: Optional[str] = field(default='nnunet')
    pretrain_vision_model: str = field(default=None, metadata={"help": "Path to pretrained model for ViT."})
    pretrain_vision_model_2d: str = field(default=None, metadata={"help": "Path to pretrained model for ViT."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    vision_encoder_3d_lr: Optional[float] = None
    mm_projector_3d_lr: Optional[float] = None
    ...
    output_dir: str = '/path/your_save_dir'
    bf16: bool = True
    per_device_train_batch_size: int = 2
    use_modality_alternating_sampler: bool = field(default=True, metadata={"help": "Whether to use modality-alternating sampler."})
...
class LazySupervisedDataset(Dataset):
    ...
    def _convert_normal(self, data_dict):
        ...
        elif 'volume' in data_dict and data_dict['volume'] is not None:
            modal = 'volume'
            if all(not "<volume>" in sentence["value"] for sentence in conversation):
                warnings.warn(f"Volume tag not found in the conversation, add it automatically at the beginning!")
                conversation[0]["value"] = "<volume>" + conversation[0]["value"]
            video_file = data_dict['volume']
            if isinstance(video_file, list) and len(video_file) == 1:
                images = nib.load(video_file[0])
                images = images.get_fdata()
                images = torch.from_numpy(images).float().unsqueeze(0)
                images = [images]
            else:
                raise ValueError(f"Unsupported 3D format: {video_file}")
        else:
            modal = 'text'
            images = None
        ...
        for conv in conversation:
            if conv["from"] == "human":
                chunks = conv["value"].split(
                    "<image>" if modal == 'image' else ("<video>" if modal == 'video' else "<volume>")
                )
                messages.append({
                    "role": "user",
                    "content": []
                ...
                for chunk_idx in range(1, 2 * len(chunks)):
                    if chunk_idx % 2 == 1:
                        ...
                        if modal == 'image':
                            ...
                            messages[-1]["content"].append({"type": "video", "num_frames": 12})
            else:
                messages.append({
                    "role": "assistant",
                    "content": conv['value']
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if modal == 'text':
                ...
                assert len(data_dict['pixel_values']) > 0
...
def train(attn_implementation=None):
    ...
    if model_args.vision_encoder is not None:
        ...
        vision_encoder = model.get_vision_encoder(modal='image')
        vision_encoder.to(dtype=compute_dtype, device=training_args.device)
        
        vision_encoder_3d = model.get_vision_encoder(modal='volume')
        vision_encoder_3d.to(dtype=compute_dtype, device=training_args.device)

        mm_max_length = data_args.mm_max_length
        vision_encoder.image_processor.max_tokens = mm_max_length

        mm_projector = model.get_mm_projector(modal='image')
        mm_projector.to(dtype=compute_dtype if training_args.bf16 else torch.float16, device=training_args.device)
        
        mm_projector_3d = model.get_mm_projector(modal='volume')
        mm_projector_3d.to(dtype=compute_dtype if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        model.config.vision_encoder_3d_lr = training_args.vision_encoder_3d_lr
        model.config.mm_projector_3d_lr = training_args.mm_projector_3d_lr
        ...
        if model.config.llm_lr is None:
            ...
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = True
            for p in model.get_model().vision_encoder_3d.parameters():
                p.requires_grad = True
            for p in model.get_model().mm_projector_3d.parameters():
                p.requires_grad = True
        ...
        if model.config.vision_encoder_3d_lr is None:
            for p in model.get_model().vision_encoder_3d.parameters():
                p.requires_grad = False

        if model.config.mm_projector_3d_lr is None:
            for p in model.get_model().mm_projector_3d.parameters():
                p.requires_grad = False
        ...
        model.config.ct_vision_encoder = model_args.ct_vision_encoder

        vlprocessor = RadSightProcessor(vision_encoder.image_processor, tokenizer)
    ...
    # select a Trainer
    trainer = RadSightTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)

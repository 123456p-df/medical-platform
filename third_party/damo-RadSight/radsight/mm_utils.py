...
def load_3D(image_path, keys=None, image_only=False):
    """
    Load 3D medical images (CT/MRI) from file path or dictionary.
    
    Args:
        image_path (str or dict): Path to 3D image file or dictionary with paths
        keys (list, optional): Keys to load from dictionary. Defaults to None (all keys).
        image_only (bool, optional): Whether to load image data only. Defaults to False.
    
    Returns:
        dict: Dictionary containing loaded image data and metadata
    """
    ...
    keys = list(image_dict.keys()) if keys is None else keys
    
    transform = Compose([
        LoadImaged(keys=keys, image_only=image_only, ensure_channel_first=True),
    ])
    images = transform(image_dict)
    return images

...
<template>
  <div class="viewer">
    ...
    <div class="viewer-info">
      <span>{{ examination.type }} · {{ examination.organ }}</span>
      <span>Slice {{ currentSlice }} / {{ examination.sliceCount }}</span>
    </div>
    <div class="viewer-controls">
      <button type="button" :disabled="!canGoBack" aria-label="Previous slice" @click="changeSlice(-1)">
        <ChevronLeft :size="17" />
      </button>
      <input
      ...
      />
      <button type="button" :disabled="!canGoForward" aria-label="Next slice" @click="changeSlice(1)">
        <ChevronRight :size="17" />
      </button>
      <span class="control-divider" />
      <button type="button" aria-label="Zoom out" @click="changeZoom(-0.2)">
        <Minus :size="16" />
      </button>
      <span class="zoom-value">{{ Math.round(zoom * 100) }}%</span>
      <button type="button" aria-label="Zoom in" @click="changeZoom(0.2)">
        <Plus :size="16" />
      </button>
      <button type="button" aria-label="Reset view" @click="zoom = 1">
        <Crosshair :size="16" />
      </button>
    </div>
  </div>

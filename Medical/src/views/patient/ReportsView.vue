...
<template>
  <div class="page">
    <PageHeader title="My Reports" subtitle="Doctor-reviewed results, written for you.">
      <template #actions>
        <span class="report-count"><FileCheck2 :size="15" /> {{ store.reviewedReports.length }} reviewed</span>
      </template>
    </PageHeader>
    ...
    <div v-if="!store.reviewedReports.length && !store.loading" class="card empty-state">
      No reviewed reports are available yet.
    </div>
  </div>
</template>

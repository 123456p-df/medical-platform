...
## 2026-09-14 患者入口与侧栏激活修复

- 患者首页不再请求医生专用患者名册接口，真实患者账号进入首页时不会触发该接口的 403。
- 医生临床工作流导航改用精确路由匹配；患者侧栏合并指向同一地址的“首页/我的健康”，任一路由只高亮一个入口。
- 类型、双语和无障碍结构检查通过；演示流程 Playwright 5 项及真实 FastAPI 患者登录 1 项通过。

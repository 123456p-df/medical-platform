...
function raycastAt(e:MouseEvent | PointerEvent,targets:THREE.Object3D[]) {
...
function pointerUp(e: PointerEvent) {
  ...
  down = null
  const hit=raycastAt(e,organs)
  if(hit) emit('select',hit.object.userData.organId)
}
function focusAtPointer(e:MouseEvent) {
  if(!camera||!controls)return
  const hit=raycastAt(e,organs) ?? raycastAt(e,shells)
  if(!hit)return
  const direction=camera.position.clone().sub(controls.target).normalize()
  const distance=Math.max(.8,Math.min(2.15,camera.position.distanceTo(controls.target)*.55))
...
onMounted(async () => {
  try {
    ...
    controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.enablePan=true;controls.screenSpacePanning=true;controls.zoomToCursor=true
    controls.minDistance=.45;controls.maxDistance=8;controls.autoRotateSpeed=.7;controls.minPolarAngle=.20;controls.maxPolarAngle=Math.PI-.20;reset()
    ...
    renderer.domElement.addEventListener('pointerdown',pointerDown);renderer.domElement.addEventListener('pointerup',pointerUp);renderer.domElement.addEventListener('dblclick',focusAtPointer)
    animate()
...
onBeforeUnmount(()=> {disposed=true;cancelAnimationFrame(frame);observer?.disconnect();controls?.dispose();renderer?.domElement.removeEventListener('pointerdown',pointerDown);renderer?.domElement.removeEventListener('pointerup',pointerUp);renderer?.domElement.removeEventListener('dblclick',focusAtPointer);if(scene)disposeObject(scene);renderer?.dispose();renderer?.domElement.remove()})
</script>
<template>

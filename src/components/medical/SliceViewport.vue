...
const props = defineProps<{ examination: Examination; axis: SliceAxis; preset: string; zoom: number; renderer: VolumeRenderer | null; blocked?: boolean }>()
...
onMounted(render)
function move(value: number) { slice.value = Math.min(geometry.value.count - 1, Math.max(0, value)) }
...
<template>
  <div :class="['slice-viewport', axis]" @keydown="keydown">
    ...
    <div ref="stage" class="slice-stage" tabindex="0" :aria-label="label + '视图，方向键切层'" @wheel.prevent="scroll" @pointerdown="stage?.focus({ preventScroll: true })">
      ...
      <div class="slice-fit" :style="{ aspectRatio: String(geometry.width / geometry.height), width: `min(100%, ${260 * geometry.width / geometry.height}px)`, transform: `scale(${zoom})` }"><canvas ref="canvas" role="img" :aria-label="examination.type + ' ' + axis + ' 切片'" :data-slice-index="displayed" :data-render-mode="renderer ? 'local' : 'preview'" /></div>
    ...
    <div class="slice-controls"><button :disabled="slice === 0 || blocked" :aria-label="label + '上一层'" @click="move(slice - 1)"><ChevronLeft :size="14" /></button><input v-model.number="slice" :disabled="blocked" :aria-label="label + '切片位置'" type="range" min="0" :max="geometry.count - 1" /><button :disabled="slice >= geometry.count - 1 || blocked" :aria-label="label + '下一层'" @click="move(slice + 1)"><ChevronRight :size="14" /></button></div>
  </div>
</template>
<style scoped>
.slice-viewport{min-width:0;overflow:hidden;border:1px solid #293943;border-radius:9px;background:#0b151d}.pane-heading{display:flex;justify-content:space-between;align-items:center;border-top:2px solid #cc7a73;padding:10px 13px;color:#d9e7e7;font-size:11px}.coronal .pane-heading{border-color:#91bb9b}.sagittal .pane-heading{border-color:#cdb776}.pane-heading span{color:#7e999f;font-size:10px}.slice-stage{height:288px;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#050b10;padding:14px}.slice-stage:focus-visible{outline:2px solid #72aaa3;outline-offset:-3px}.slice-fit{max-height:260px;flex-shrink:0}.slice-fit canvas{display:block;width:100%;height:100%}.orientation{position:absolute;z-index:2;color:#91aab0;font-size:10px;pointer-events:none}.top{top:6px;left:50%}.bottom{bottom:6px;left:50%}.left{top:50%;left:8px}.right{top:50%;right:8px}.slice-message{position:absolute;z-index:3;color:#d1e3e7;font-size:11px;padding:10px;background:#0f212bea;border-radius:6px;max-width:95%;text-align:center}.slice-message button{display:block;margin:10px auto 0}.slice-caption{position:absolute;bottom:10px;left:12px;color:#aac3c7;font-size:10px;background:#0a1e25bf;padding:3px 6px;border-radius:3px}.slice-controls{display:flex;align-items:center;gap:8px;padding:6px 10px}.slice-controls input{flex:1;min-width:0;accent-color:#72aaa3}.slice-controls button{border:0;background:none;color:#a9c1c5;padding:5px;display:flex}.slice-controls button:disabled{opacity:.3}
</style>

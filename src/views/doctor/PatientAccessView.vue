...
async function grantAccess() {
  const patientId = Number(newPatientId.value)
  if (!selectedDoctorId.value || !Number.isInteger(patientId) || patientId <= 0) {
    error.value = t('ui.patientAccess.invalidPatient')
    return
  }
  busy.value = true
  error.value = ''
  try {
    if (localPreview) {
      const existing = rows.value.find(item => item.patientId === patientId)
      if (existing) existing.status = 'active'
      else rows.value.unshift({
        doctorUserId: selectedDoctorId.value,
        doctorUsername: selectedDoctor.value?.username || '',
      ...
      await adminApi.setPatientAccess(selectedDoctorId.value, patientId, 'active')
      await loadAccess()
    }
    newPatientId.value = ''
    message.value = t('ui.patientAccess.granted')
  } catch (reason) {
    error.value = reason instanceof Error ? reason.message : t('ui.patientAccess.updateFailed')
  } finally {
    busy.value = false
...
<template>
  <div class="admin-page">
    ...
    <section class="card filter-card">
      ...
      <label class="label">{{ $t('ui.patientAccess.patientId') }}<input v-model="newPatientId" class="input" inputmode="numeric" :placeholder="$t('ui.patientAccess.patientIdPlaceholder')" /></label>
      <button class="btn btn-primary" type="button" :disabled="busy || !selectedDoctorId || !newPatientId" @click="grantAccess"><Link2 :size="16" /> {{ $t('ui.patientAccess.grant') }}</button>

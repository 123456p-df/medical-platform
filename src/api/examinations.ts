...
export const examinationApi = {
  ...
  async uploadDicomSeries(
    ...
    return new Promise<ReturnType<typeof mapImage>>((resolve, reject) => {
      const xhr = new XMLHttpRequest()
      const abort = () => xhr.abort()
      xhr.open('POST', '/api/v1/patients/' + patientId + '/dicom-images')
      xhr.timeout = 10 * 60_000
      const accessToken = token()
      if (accessToken) xhr.setRequestHeader('Authorization', 'Bearer ' + accessToken)
      xhr.upload.onprogress = event => {
        const totalBytes = event.lengthComputable ? event.total : total
        const percent = totalBytes > 0 ? Math.min(100, Math.round(event.loaded / totalBytes * 100)) : 0
        options.onProgress?.({ phase: 'uploading', loaded: event.loaded, total: totalBytes, percent })
      }
      xhr.upload.onload = () => options.onProgress?.({ phase: 'processing', loaded: total, total, percent: 100 })
      xhr.onload = () => {
        options.signal?.removeEventListener('abort', abort)
        let payload: { data?: ImageDTO; code?: number; message?: string } = {}
        try { payload = JSON.parse(xhr.responseText || '{}') }
        ...
        reject(new ApiError(xhr.status, payload.code || 0, payload.message || 'DICOM 上传失败', { retryable: xhr.status === 0 || xhr.status >= 500 }))
      }
      xhr.onerror = () => reject(new ApiError(0, 0, 'DICOM 上传连接中断，请重试。', { retryable: true }))
      xhr.ontimeout = () => reject(new ApiError(0, 40800, 'DICOM 上传超时，请重试。', { retryable: true }))
      xhr.onabort = () => reject(new DOMException('上传已取消', 'AbortError'))
      options.signal?.addEventListener('abort', abort, { once: true })
      if (options.signal?.aborted) abort()
      else xhr.send(form)
